...
def main():
    """
    Create or replace an OANDA Trailing Stop Loss Order in an Account based on
    ...
    if args.replace_order_id is not None:
        ...
        #
        response = api.order.trailing_stop_loss_replace(
        ...
        #
        response = api.order.trailing_stop_loss(
            args.config.active_account,
            **orderArgs.parsed_args
        )
